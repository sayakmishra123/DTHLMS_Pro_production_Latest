...
class MockTestresult extends StatefulWidget {
 ...
 final String submitedOn;
  ...
  const MockTestresult({required this.paperId, required this.questionData,required this.totalmarks,required this.type, required this.submitedOn, required this.paperName, required this.isAnswerSheetShow,});
  @override
...
class _MockTestresultState extends State<MockTestresult>
  ...
  print(widget.isAnswerSheetShow.toString()+"/////////////////////////");
   ...
   double.parse(totalRequiredMarks)< calculateTotalMarks(widget.questionData) ? _animationController.forward() : null; 
  }
        ...
        body: Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: Container(
                  ...
                  child: Stack(
                    children: [
                      SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: ColorPage.colorbutton, // Main purple background color
                              ...
                              child: Row(
                                ...
                                children: [
                                  _buildStatColumn(Icons.numbers_outlined, 'Total Marks', widget.totalmarks),
                                  _buildDivider(),
                                  _buildStatColumn(Icons.star, 'Your Marks', widget.type=="Ranked Competition"?'${  getx.userRankDetails["TotalMarks"]}':usermarks),
                                  _buildDivider(),
                                  _buildStatColumn(Icons.question_mark_sharp, 'Total Question', '${widget.questionData.length}'),
                                ],
                              ),
                            ),
                            ...
                            double halfWidth = constraints.maxWidth / 2 - 11;
                            ...
                            child: Padding(
                               ...
                               children: [
                                 ...
                                 SizedBox(height: 20,),
                                      ...
                                      alignment: Alignment.centerLeft,
                                       child: RichText(
                                        ...
                                        text: TextSpan(
                                            ...
                                            TextSpan(text: getx.loginuserdata[0].firstName +" "+ getx.loginuserdata[0].lastName , style: studentNameStyle),
                                          ]),
                                          ),
                                     ),
                                    ...
                                    child: RichText(text: TextSpan(
                                          ...
                                          children: [
                                          TextSpan(text: 'Submitted on: ', style: studentTitleStyle),
                                                        
                                          TextSpan(text: formatDate(widget.submitedOn), style: studentNameStyle),
                                        ]),
                                        ),),
                             ...
                             children: [
                              RichText(text: TextSpan(
                                    ...
                                    TextSpan(text:double.parse(totalRequiredMarks)< calculateTotalMarks(widget.questionData)?"Pass":"Fail" , style: TextStyle(fontSize: 18,color: double.parse(totalRequiredMarks)< calculateTotalMarks(widget.questionData)? Colors.green : Colors.redAccent))
                             ...
                             width: double.infinity,
                              child: DataTable(
                                ...
                                rows: <DataRow>[
                                  DataRow(
                                    cells: <DataCell>[
                                      DataCell(Text(widget.paperName)),
                                     
                                      DataCell(Text(widget.totalmarks)),
                                      DataCell(Text(calculateTotalMarks(widget.questionData).toString())),
                                    ],
                                  ),
                                ],
                              ),
                            ),
                            SizedBox(height: 20), // Space between the table and the row
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceAround,
                              children: [
                                RichText(text: TextSpan( 
                                  ...
                                  TextSpan(text: totalRequiredMarks, style: TextStyle(fontSize: 16,color: Colors.blue)),
                                ]),
                                ),
                              ],
                            ), 
                           ...
                           widget.type=="Ranked Competition" ?  Container(
                              height: 40,
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                         ...
                         widget.type=="Ranked Competition" ?  SizedBox(
                           ...
                           child: Obx(()=>
                              ListView.builder(
                               ...
                               itemBuilder: (context, index) {
                                  print(getx.rankerList.length.toString());
                    ...
                    widget.type=="Ranked Competition"?  Positioned(
                        bottom: 20,
                        left: 5,
                        right: 5,
                        child: Container(
              ...
              Expanded(
                child: Container(
                  child:widget.isAnswerSheetShow? ListView.builder(
                      ...
                      }):Center(child: Text(" The answer sheet is not authorized for display "),)
                ),
              ),
...
double remainingMarks =double.parse(widget.totalmarks) -calculateTotalMarks(widget.questionData) ;
...
if(type=="One Word Answer"){
  String answer =getOneWordAnswerFromQuestionId(questionId.toString()).replaceAll(" ", "");
  print(answer);
  String userAns=getOneWordAnswerFromQuestionIdofStudent(questionId.toString()).replaceAll(" ", "");
  print(userAns);
  if(answer.toLowerCase()==userAns.toLowerCase()){
    totalMark+=fetchMCQPartialCorrectMarks(userSelectedQuestionId[0].toString(), questionId);

  }
  else{
      totalMark+=fetchMCQPartialNegativeMarks(userSelectedQuestionId[0].toString(), questionId);

  }
   double negetiveMarks=fetchnagetivemarksFromsection(fetchsectionIdfromquestion(questionId));
return negetiveMarks*-1>totalMark?negetiveMarks*-1: totalMark;
}
else{
    for (var i = 0; i < userSelectedQuestionId.length; i++) {

totalMark+=fetchMCQPartialCorrectMarks(userSelectedQuestionId[i].toString(), questionId)-fetchMCQPartialNegativeMarks(userSelectedQuestionId[i].toString(), questionId);
...
double calculateTotalMarks(List questionData){
  ...
  for(var data in questionData){
    totalMarks+=calculateTotalMarkOfQuestion(data['questionid'].toString(),data['userselectedanswer id'],getQuestionTypeFromId(data['questionid'].toString()));
